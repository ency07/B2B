...
export function TopBar({ siteName, logoUrl, tenantCode }: TopBarProps) {
  ...
  return (
    <>
      <header
        className={`
          fixed top-0 left-0 right-0 z-layer-modal
        ...
        {open && (
          <>
            <motion.div
              ...
              className="fixed inset-0 z-layer-sticky bg-ink/60 backdrop-blur-sm"
            />
            <motion.aside
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              ...
              className="fixed top-0 right-0 bottom-0 z-layer-modal w-full sm:w-[88%] sm:max-w-md bg-paper border-l border-line flex flex-col"
            >
