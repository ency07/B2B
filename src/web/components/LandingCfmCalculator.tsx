...
export default function LandingCfmCalculator() {
  ...
  return (
    <div className="w-full bg-paper-cool dark:bg-zinc-950 border border-line dark:border-zinc-800/50 rounded-md overflow-hidden transition-colors duration-300">
      <div className="border-b border-line dark:border-zinc-800/50 p-6 bg-paper-warm dark:bg-zinc-900/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <span className="text-xs font-mono text-ink-muted dark:text-zinc-400 uppercase tracking-widest">Motor de Prediseño</span>
          <h3 className="text-lg font-bold text-ink dark:text-white font-display mt-1">Calculadora Técnica de Preingeniería</h3>
        </div>
      ...
      <div className="grid lg:grid-cols-12 gap-0 divide-y lg:divide-y-0 lg:divide-x divide-line dark:divide-zinc-800/50">
        
        {/* Lado Controles (Entradas) */}
        <div className="lg:col-span-6 p-6 md:p-8 space-y-6">
          <h4 className="text-xs font-bold uppercase tracking-wider text-ink-muted dark:text-zinc-400 font-mono mb-4">1. Parámetros Físicos y Ambientales</h4>
          
          {/* Dimensiones */}
          <div className="space-y-4">
            <div>
              <div className="flex justify-between text-xs font-medium text-ink-soft dark:text-zinc-300 mb-1">
                <span>Largo de la Nave:</span>
                <span className="font-bold text-ink dark:text-white font-mono">{largo} m</span>
              </div>
              <input 
            ...
            <div>
              <div className="flex justify-between text-xs font-medium text-ink-soft dark:text-zinc-300 mb-1">
                ...
                <span className="font-bold text-ink dark:text-white font-mono">{ancho} m</span>
              </div>
              <input 
            ...
            <div>
              <div className="flex justify-between text-xs font-medium text-ink-soft dark:text-zinc-300 mb-1">
                ...
                <span className="font-bold text-ink dark:text-white font-mono">{alto} m</span>
              </div>
              <input 
          ...
          <div className="grid grid-cols-2 gap-4 pt-4 border-t border-line dark:border-zinc-900/60">
            {/* Sector */}
            <div>
              <label className="text-xs font-semibold text-ink-muted dark:text-zinc-400 block mb-1">Sector Operativo:</label>
              <select 
                value={sector} 
                onChange={(e) => setSector(e.target.value)}
                className="w-full bg-zinc-50 dark:bg-zinc-900 border border-line dark:border-zinc-800 rounded-md p-2.5 text-xs text-ink dark:text-white focus:outline-none focus:border-[var(--brand-primary)]"
              >
            ...
            {/* Red de ductos */}
            <div>
              <label className="text-xs font-semibold text-ink-muted dark:text-zinc-400 block mb-1">¿Requiere Red de Ductos?</label>
              <select 
                value={ducts} 
          ...
          <div className="grid grid-cols-2 gap-4 pt-4 border-t border-line dark:border-zinc-900/60">
            ...
            <div>
              <label className="text-xs font-semibold text-ink-soft dark:text-zinc-300 block mb-1">Altitud (msnm):</label>
              <select
                value={altitude}
            ...
            {/* Temperatura */}
            <div>
              <div className="flex justify-between text-xs font-semibold text-ink-soft dark:text-zinc-300 mb-1">
                <span>Temperatura Ambiente:</span>
                <span className="font-bold text-ink dark:text-white font-mono">{temp} °C</span>
              </div>
              <input 
        ...
        <div className="lg:col-span-6 p-6 md:p-8 bg-paper-warm/50 dark:bg-zinc-950/30 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {/* Volumen */}
            <div className="bg-white dark:bg-zinc-900/10 border border-line dark:border-zinc-800/80 rounded-md p-4 flex flex-col justify-between shadow-2xs">
              <span className="text-xs text-ink-muted dark:text-zinc-400 uppercase tracking-widest font-mono">Volumen a Tratar</span>
              <div className="text-lg font-bold text-ink dark:text-white mt-1 font-mono">
                {volume.toLocaleString()} <span className="text-xs text-ink-muted dark:text-zinc-400 font-normal">m³</span>
              </div>
            </div>

            {/* ACH */}
            <div className="bg-white dark:bg-zinc-900/10 border border-line dark:border-zinc-800/80 rounded-md p-4 flex flex-col justify-between shadow-2xs">
              <span className="text-xs text-ink-muted dark:text-zinc-400 uppercase tracking-widest font-mono">Renovaciones (ACH)</span>
              <div className="text-lg font-bold text-[var(--brand-primary)] mt-1 font-mono">
                {ach} <span className="text-xs text-ink-muted dark:text-zinc-400 font-normal">renov/h</span>
              </div>
            </div>

            {/* CFM */}
            <div className="bg-white dark:bg-zinc-900/10 border border-line dark:border-zinc-800/80 rounded-md p-4 col-span-2 flex flex-col justify-between relative overflow-hidden shadow-2xs">
              ...
              <div className="text-4xl font-extrabold text-[var(--brand-primary)] mt-1 font-mono">
                {cfm.toLocaleString()} <span className="text-sm text-ink-muted font-normal">CFM</span>
              </div>
            </div>

            {/* Consumo y Potencia */}
            <div className="bg-white dark:bg-zinc-900/10 border border-line dark:border-zinc-800/80 rounded-md p-4 flex flex-col justify-between shadow-2xs">
              <span className="text-xs text-ink-muted dark:text-zinc-400 uppercase tracking-widest font-mono">Potencia del Motor</span>
              <div className="text-base font-bold text-ink dark:text-white mt-1 font-mono">
                {powerHp} <span className="text-xs text-ink-muted dark:text-zinc-400 font-normal">HP</span>
                <span className="text-xs text-ink-muted dark:text-zinc-400 block font-normal mt-0.5">({powerKw} kW/h consumo)</span>
              </div>
            </div>

            {/* Ruido y Presión */}
            <div className="bg-white dark:bg-zinc-900/10 border border-line dark:border-zinc-800/80 rounded-md p-4 flex flex-col justify-between shadow-2xs">
              <span className="text-xs text-ink-muted dark:text-zinc-400 uppercase tracking-widest font-mono">Ruido y Presión</span>
              <div className="text-base font-bold text-ink dark:text-white mt-1 font-mono">
                {noiseDba} <span className="text-xs text-ink-muted dark:text-zinc-400 font-normal">dBA</span>
                <span className="text-xs text-ink-muted dark:text-zinc-400 block font-normal mt-0.5">({staticPressure} inWG Presión)</span>
              </div>
            </div>

            {/* Equipos sugeridos */}
            <div className="bg-white dark:bg-zinc-900/10 border border-line dark:border-zinc-800/80 rounded-md p-4 col-span-2 space-y-2 shadow-2xs">
              <div className="flex justify-between items-center border-b border-line dark:border-zinc-800 pb-2">
                <span className="text-xs text-ink-muted dark:text-zinc-400 uppercase tracking-widest font-mono">Cantidad de Equipos:</span>
                <span className="text-xs font-bold text-ink dark:text-white font-mono">{qtyEquip} Unidades</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-ink-muted">Distribución Sugerida:</span>
                <span className="text-ink-soft dark:text-ink-soft font-semibold">{distrib}</span>
              </div>
              <div className="flex justify-between items-center text-xs border-t border-line dark:border-zinc-800 pt-2">
                <span className="text-ink-muted">Inversión Estimada (COP):</span>
                <span className="text-state-success font-bold font-mono">{investment}</span>
              </div>
            </div>
          </div>

          {/* Alerta de criticidad */}
          <div className="flex items-center gap-3 p-3 bg-paper-warm/70 dark:bg-zinc-900/50 rounded-md border border-line dark:border-zinc-800/50 text-ink-soft dark:text-zinc-300">
            <ShieldAlert className="w-5 h-5 text-state-warning shrink-0" />
            <div className="text-xs leading-relaxed">
              <span className="font-bold text-state-warning block font-mono">SEVERIDAD: {criticality}</span>
