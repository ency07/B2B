-- =============================================================================
-- PORTAL REQUIREMENTS — Flujo de requerimientos iniciados desde el portal
-- Fuente: 'PORTAL' — trazable en el ERP desde requirements.source
-- =============================================================================

-- ---------------------------------------------------------------------------
-- portal_get_client_requirements
-- Lista los requerimientos de un cliente; sigue el patrón de portal_get_client_*
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  ELSE
    ...
    WHERE cc.user_id = auth.uid() AND cc.deleted_at IS NULL
    LIMIT 1;
  END IF;
...
-- ---------------------------------------------------------------------------
-- portal_create_client_requirement
-- Crea un requerimiento con status='NUEVO' y source='PORTAL'
-- created_by usa el primer usuario del tenant (el portal no tiene user en
-- la tabla interna de users — source='PORTAL' marca el origen real)
-- ---------------------------------------------------------------------------
